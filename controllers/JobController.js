...
const createJob = async (req, res) => {
    ...
    try {
        if ([title, description, category, location, salary].some(value => value === undefined || value === null || String(value).trim() === '')) {
            return res.json({ success: false, message: 'لطفا تمام موارد بالا را پر کنید' })
        }
...
const completeJob = async (req, res) => {
    ...
    try {
        if (!workerId) return res.json({ success: false, message: 'کارگر انتخاب شده مشخص نیست' })

        const job = await JobModel.findOne({ _id: jobId, createdBy: userId })
        if (!job) return res.json({ success: false, message: 'شما صاحب این کار نمیباشید' })
        if (job.status === 'completed') return res.json({ success: false, message: 'این کار قبلاً ختم شده است' })
...
const getApplications = async (req, res) => {
    ...
    try {
        const applications = await ApplicationModel.find({ job: jobId })
            .populate('worker', 'profileImage name job expereinceYear province district rate')
            .sort({ createdAt: -1 })
        if (!applications.length) return res.json({ message: 'درخواستی برای این کار ثبت نشده است', success: false, data: [] })
